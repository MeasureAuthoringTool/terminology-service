...
class FhirTerminologyServiceTest {
  ...
  void testRetrieveAllCodeSystemsWithInsert() {
    ...
    when(fhirTerminologyServiceWebClient.getCodeSystemsPage(anyInt(), anyInt(), anyString()))
            .thenReturn(mockCodeSystemsResource);
    var existingCodeSystem = gov.cms.madie.terminology.models.CodeSystem.builder().id("titleversion")
    .title("title").name("name1").version("version").versionId("vid").oid("codeUrl").lastUpdated(Instant.now()).lastUpdatedUpstream(new Date()).build();

    when(codeSystemRepository.findById(anyString())).thenReturn(Optional.ofNullable(existingCodeSystem));
