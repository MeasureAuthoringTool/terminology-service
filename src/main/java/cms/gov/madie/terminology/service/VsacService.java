...
public class VsacService {
  ...
  public List<CqlCode> validateCodes(List<CqlCode> cqlCodes, String tgt) {
    ...
    for (CqlCode cqlCode : cqlCodes) {
      ...
      if (cqlCodeSystemOid != null && !cqlCodeSystemOid.isBlank()) {
        Optional<CodeSystemEntry> codeSystemEntry =
            codeSystemEntries.stream()
                .filter(cse -> cse.getUrl().equalsIgnoreCase(cqlCodeSystemOid.replace("'", "")))
                .findFirst();
        if (codeSystemEntry.isPresent()) {
          if (codeSystemEntry.get().getOid().contains("NOT.IN.VSAC")) {
            // if codeSystemEntry is available in mapping json, but listed as NOT IN VSAC, then it is a valid FHIR code system.
            cqlCode.setValid(true);
          } else {
            String codeSystemVersion = buildCodeSystemVersion(cqlCode, codeSystemEntry.get());
            if (cqlCode.getCodeId() == null || cqlCode.getCodeId().isBlank()) {
          ...
          log.debug("No associated Code system found in code system entry json for {}", cqlCode.getCodeSystem().getOid());
   ...
   * @return if user provides a FHIR Code system version, find the equivalent VSAC version in codeSystemEntry and returns it.
   *         if there is no equivalent version in codeSystemEntry, or there are no versions at all in codeSystemEntry,
   *         then this method returns user provided FHIR version.
   *         Finally, if user doesn't provide a version in CQL, get the latest vsac version from codeSystemEntry (first element in the version List).
   */
  private String buildCodeSystemVersion(CqlCode cqlCode, CodeSystemEntry codeSystemEntry) {
    String cqlCodeSystemVersion = cqlCode.getCodeSystem().getVersion().replaceAll("'", "");
    List<CodeSystemEntry.Version> codeSystemEntryVersion = codeSystemEntry.getVersion();
    if (!cqlCodeSystemVersion.isBlank()) {
      if (codeSystemEntryVersion == null || codeSystemEntryVersion.isEmpty()) {
        log.debug("CodeSystem {} does not have any known versions", cqlCode.getCodeSystem().getOid());
        ...
        if (optionalCodeSystemVersion.isPresent()) {
          ...
          log.debug("None of the known FHIR code system versions {} matches with version {}",
              cqlCode.getCodeSystem().getOid(), cqlCodeSystemVersion);
          return cqlCodeSystemVersion;
        }
      }
    } else {
      if (codeSystemEntryVersion == null || codeSystemEntryVersion.isEmpty()) {
        throw new RuntimeException(
            String.format("CodeSystem %s does not have any known versions", cqlCode.getCodeSystem().getOid()));
      }
